...
from free_energy_principle import free_energy_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    for ticker in ret_window.columns:
        ...
        s = free_energy_score(
            ...
            generative_dim=config.GENERATIVE_DIM,
            iterations=config.FREE_ENERGY_ITERATIONS
        )
    ...
    top_etfs = [{"ticker": t, "fep_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    results = {
        ...
        "n_agents": config.N_AGENTS,
        "generative_dim": config.GENERATIVE_DIM,
        "free_energy_iterations": config.FREE_ENERGY_ITERATIONS,
    ...
    out_file = f"output/fep_{timestamp}.json"
