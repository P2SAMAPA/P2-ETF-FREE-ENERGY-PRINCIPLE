...
st.set_page_config(page_title="Free Energy Principle – Market Beliefs", layout="wide")
...
st.sidebar.markdown(f"**Agents:** {config.N_AGENTS} | **Generative dim:** {config.GENERATIVE_DIM}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'fep_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Market belief: {etf['fep_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Belief"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Belief", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🧠 Top ETFs by Market Belief Coherence (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Free Energy Principle** (Karl Friston) frames markets as systems minimising variational free energy.
        - **Active inference** agents maintain internal generative models of the market.
        - Price dynamics emerge from collective belief updating.
        - **Variational free energy** = surprise - coherence.
        - Higher belief coherence → more stable market regime → potential alpha.
        - Lower free energy → more coherent market beliefs.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the market belief scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Free Energy Principle | Active inference for market belief coherence")
